...
```{r}
rm(ucoa_fund, ucoa_funct, ucoa_account, extra_fund_numbers, ucoa_account_exp,
   ucoa_account_rev )
```
...
left_join(ucoa_lookup[["funct"]] %>%
            as_tibble() %>%
            select(NUMBER, funct_description = `FULL DESCRIPTION`),
          by = c("funct" = "NUMBER")
          ) %>% 
# join the accounts
...
```{r}
test<- barplot_description_amount(templist = osa_agg_tables,
               agg_by_type= "agg_by_description_and_code",
               agg_type = "agg_funct",
               uca_block = funct,
               uca_block_description = funct_description,
...
```{r}
invalid_tables %>% pluck("unmapped_funct_code") %>% select(id, batch_id, amount, funct, funct_description)
```
...
```{r}
invalid_tables %>% pluck("lea_unmapped_funct_code") %>% select(id, batch_id, amount, funct, funct_description)
```
