...
query_transactions <- function(batches_list, lookup_table) {
  ...
  invalid_form <-
    ...
    mutate(
      reason = "uca code incorrect form. form must be :###-######-########"
    )
  ...
  # invalid_transaction_type <- transaction_report %>% 
  #   filter(type ==4 || type ==5 || type ==6 || type== 7) %>% 
  #   mutate(
  ...
  #     
  #     reason =  case_when(
  #       type == 4 ~ "transaction type correpsonds to stimulus expense",
  #       type == 5 ~ "transaction type correpsonds to stimulus revenue",
  #       type == 6 ~ "transaction type correpsonds to budget",
  #       type == 7 ~ "transaction type correpsonds to balance sheet"
  #     )
  #     
  #   )
  # 
  # transaction_report <- transaction_report %>% 
  #   anti_join(invalid_transaction_type, by="id")
  
  invalid_transaction_type <- tibble()
  ...
  unmapped_fund_code <- transaction_report %>%
    ...
    mutate(
      reason = "The fund code does not exist in the current uniform chart of accounts. "
    )
  ...
  unmapped_fund_code_primary <- unmapped_fund_code %>%
    ...
    mutate(
      reason = "The primary piece of the fund code does not exist in the current uniform chart of accounts. "
    )
  ...
  unmapped_fund_code_primary <- unmapped_fund_code %>%
    ...
    mutate(
      reason = "The secondary piece the fund code does not exist in the current uniform chart of accounts. "
    )
  ...
  unmapped_funct_code <- transaction_report %>%
    ...
    mutate(reason = "The function code does not exist in the current uniform chart of accounts. ")
  ...
  unmapped_funct_code_primary <- unmapped_funct_code %>%
    ...
    mutate(reason = "Invalid function code - The first and second digits of the function code do not exist in the current uniform chart of accounts. ")
  ...
  unmapped_funct_code_secondary <- unmapped_funct_code %>%
    ...
    mutate(reason = "Invalid function code -The third and fourth digits of the function code do not exist in the current uniform chart of accounts. ")
  ...
  unmapped_funct_code_tertiary <- unmapped_funct_code %>%
    ...
    mutate(reason = "Invalid function code - The fifth and sixth digits of the function code do not exist in the current uniform chart of accounts. ")
  ...
  unmapped_account_code <- transaction_report %>%
    ...
    mutate(reason = "The account code does not exist in the current uniform chart of accounts. ")
  ...
  unmapped_account_code_primary <- unmapped_account_code %>%
    ...
    mutate(reason = "Invalid account code - The first and second digits of account code do not exist in the current uniform chart of accounts. ")
  ...
  unmapped_account_code_secondary <- unmapped_account_code %>%
    ...
    mutate(reason = "Invalid account code - The third and fourth digits of the account code do not exist in the current uniform chart of accounts. ")
  ...
  unmapped_account_code_tertiary <- unmapped_account_code %>%
    ...
    mutate(reason = "Invalid account code - The fifth and sixth digits of the account code do not exist in the current uniform chart of accounts. ")
  ...
  unmapped_account_code_quaternary <- unmapped_account_code %>%
    ...
    mutate(reason = "Invalid account code -The seventh and eighth digits of the account code do not exist in the current uniform chart of accounts. ")
  ...
  transaction_report <- transaction_report %>%
    ...
    #finally check that the account is mapped to the correct type. 
  # 1= expediture
  # 2 = revenue
  # 
  exp_transaction_but_rev_account <- transaction_report %>%
    filter(type == 1 && str_sub(account, 0, 1) ==3)%>%
    mutate(reason = "the transaction is listed as an expenditure, but has an account code correpsonding to a revenue ") %>% 
    select(
      id,
      batch_id,
      amount,
  ...
  rev_transaction_but_exp_account <- transaction_report %>%
    ...
    mutate(reason = "the transaction is listed as a revenue, but has an account code correpsonding to a expenditure ") %>%
    select(
      id,
      batch_id,
      amount,
  ...
  # Deal with NOT Applicable Function
  
  invalid_function_use <- transaction_report %>% 
    filter(funct == "000000") %>% 
    mutate(reason = "'Not Applicable' function code is no longer acceptable. Fix function code to have more detail.")
...
```{r}
find_precise_uca_failures <- function(entity_name,uca_summary_string, report_df){
...
```{r}
pluck_queried_transactions("Bluffdale City", bad_uca_entities)
```


```{r}
export_uca_problems(entity_name = "Bluffdale City",
                    bad_uca_entities = bad_uca_entities)
```
...
## 
file_path <- paste0("E:/rprojects/Transparency-UCOA/bad-entity-files/",entity_name,".xlsx")
