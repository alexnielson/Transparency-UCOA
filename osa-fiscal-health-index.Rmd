...
query_transactions <- function(batches_list,lookup_table) {
  ...
  #fix the year variable. We need it to be a factor.
  transaction_report<- transaction_report %>% mutate(
    # fiscal_year = forcats::fct_explicit_na(as.character(fiscal_year))
...
report <- report %>% head() %>% 
  mutate(queried_transactions = batches %>% map(query_transactions, lookup_table=osa_lookup))
